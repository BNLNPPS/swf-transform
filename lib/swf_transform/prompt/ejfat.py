...
# Runs inside the EPIC Singularity image, same as `_PYROOT_WRITER_SCRIPT`, but
# writes with `uproot` (mirroring `_write_root_events`'s host-side write)
# instead of going through PyROOT's `ak.to_rdataframe`/`Snapshot`.
_UPROOT_WRITER_SCRIPT = '''
import pickle
import sys
...
try:
    ...
    raise RuntimeError(
        "writing EJFAT events to ROOT via uproot requires the uproot package "
        "(https://github.com/scikit-hep/uproot5) inside the EPIC image, to "
...
with uproot.recreate(root_file) as f:
    f["events"] = events
print(f"wrote {len(events)} events to {root_file} via uproot")
'''
...
def _write_root_events_uproot(payload_b64, content, logger):
    """Same as `_write_root_events_pyroot`, but writes the ROOT file with
    `uproot` (mirroring `_write_root_events`'s write, and
    `fast_processing_ejfat._read_eic_root_events` on the sending side) instead
    of PyROOT's `ak.to_rdataframe`/`Snapshot`. For pilot environments with no
    local `uproot`, this still runs the write inside the EPIC Singularity
    image -- the same image ships `uproot` alongside PyROOT.
    ...
    logger.info(
        f"Writing EJFAT events to ROOT file via uproot (epic_image={epic_image}):\n{script}"
    )
    ...
    try:
        proc = subprocess.run(
            ...
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            timeout=int(content.get("uproot_write_timeout", 600)),
        )
        logger.info(f"uproot write output: {proc.stdout.decode(errors='replace')}")
    except subprocess.CalledProcessError as exc:
        output = exc.output.decode(errors="replace") if exc.output else ""
        raise RuntimeError(
            f"Failed to write ROOT file via uproot (rc={exc.returncode}): {output}"
        ) from exc
    except subprocess.TimeoutExpired as exc:
        raise RuntimeError(f"Timed out writing ROOT file via uproot: {exc}") from exc
    finally:
        for path in (pickle_file, writer_script):
            try:
                os.remove(path)
    ...
    if not os.path.exists(root_file):
        raise RuntimeError(f"uproot write completed but output file not found: {root_file}")

    logger.info(f"Wrote EJFAT events to ROOT file via uproot: {root_file}")
    return root_file
